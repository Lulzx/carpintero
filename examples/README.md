...
| --- | --- | --- |
| `json.art` | JSON in nine rules | The proposal's headline claim, and the shortest complete grammar here. Ends by showing what `scanError` says about `{"a": 01}`. |
...
`stripComments` and the rest in the tree walk. Both were several times
